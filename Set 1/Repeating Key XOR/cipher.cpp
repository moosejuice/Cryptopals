...
float cipher::languageScore(std::deque<char> input)
{
	...
	//**NOTE: This is likely not working properly and needs to be redone and tested against some test cases.
...
deque<bool> cipher::repeatingKeyXOR(deque<bool> binQue, string cipher)
{
	//This is finally working..At least with the ICE example
	//
	//
	//
	...
	int k;
	deque<bool> cipherBinQue;
	int inputSize = first.size()/8;
	if( inputSize > cipherSize)
	{
		k = (inputSize%cipherSize) - 1;
		if( inputSize%cipherSize == 0)
			k=cipherSize-1;
	}
	else if (inputSize < cipherSize)
		k = cipherSize - ((inputSize)%cipherSize);
	else
		k = cipherSize-1;
	...
	for(unsigned int i = 0; i < first.size(); i++)
	{
		if( (i%8)==0 )
		{
			...
			char t = cipher[k];
			temp = bitset<8>(int(t)).to_string();
			cipherBinQue.clear();
			for(int p = 0; p < 8; p++)
				cipherBinQue.push_back( (temp[p]=='1') );

			k--;
		}

		if( (first.at(i) + cipherBinQue.at(cipherPos)) % 2 )
			output.push_front(true);
		else
			output.push_front(false);
