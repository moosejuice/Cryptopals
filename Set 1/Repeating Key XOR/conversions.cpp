...
deque<bool> conversions::AsciiToBinary(std::vector<char> input)
{
	string binaryString;
	deque<bool> output;
	...
	{
		binaryString = bitset<8>(int(input[i])).to_string();
		for(int k = 0; k < 8; k++)
		{
			output.push_back( (binaryString[k]=='0') );
		}
	}
...
deque<bool> conversions::BinaryVectorToDeque(vector<char> input)
{
	deque<bool> output;
	for(unsigned int i = 0; i < input.size(); i++)
 ...
 string conversions::DequeToString(deque<bool> input)
 {
	 string output;
	 for(deque<bool>::reverse_iterator rit = input.rbegin(); rit != input.rend(); rit++)
		 output += input.at((*rit));
 ...
 string conversions::DequeToString(deque<char> input)
 {
	 string output;
	 for(deque<char>::reverse_iterator rit = input.rbegin(); rit != input.rend(); rit++)
		 output += input.at((*rit));
