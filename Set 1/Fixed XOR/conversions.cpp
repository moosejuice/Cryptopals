...
deque<char> conversions::XORBinaryStringEqSize(deque<char> firstQue, deque<char> secondQue)
{
	...
	for(unsigned int i = 0; i < firstQue.size(); i++)
		first.push_front( (firstQue.at(i) == '0') ? false : true );

	for(unsigned int i = 0; i < secondQue.size(); i++)
		second.push_front( (secondQue.at(i) == '0') ? false : true );
...
std::deque<char> conversions::BinaryToHex(std::deque<char> inQue)
{
	deque<char> BinQue;
	char HexChar;
	string BinStr, t;
	...
	for(deque<char>::reverse_iterator rit = inQue.rbegin(); rit != inQue.rend(); rit++)
	{
		BinStr += (*rit);
		if(BinStr.length() == 4)
		{
